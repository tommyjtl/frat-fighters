...
namespace BeatEmUpTemplate2D
{
    ...
    class UnitSettingsEditor : Editor
    {
        ...
        void MainContent(UnitSettings settings)
        {
            ...
            //attack Data (Enemy)
            if (settings.unitType == UNITTYPE.ENEMY || settings.unitType == UNITTYPE.BOSS)
            {
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button(new GUIContent(space + "Attack Data", GetArrow(foldOutList["attackDataFoldout"])), FoldOutStyle(), GUILayout.ExpandWidth(true), GUILayout.Height(30))) foldOutList["attackDataFoldout"] = !foldOutList["attackDataFoldout"];
                if (GUILayout.Button(new GUIContent("", GetInfoIcon()), FoldOutStyle(), GUILayout.Width(50), GUILayout.Height(30))) showInfo(3, settings.unitType);
                ...
                {
                    EditorGUI.indentLevel++;
                    if (settings.unitType == UNITTYPE.ENEMY || settings.unitType == UNITTYPE.BOSS) DrawPropertyField("enemyPauseBeforeAttack");
                    EditorGUILayout.Space(5);
            ...
            {
                EditorGUI.indentLevel++;
                if (settings.unitType == UNITTYPE.ENEMY || settings.unitType == UNITTYPE.BOSS) DrawPropertyFields(defenceFieldsEnemy);
            ...
            if (foldOutList["grabFoldout"])
            {
                ...
                if (settings.unitType == UNITTYPE.ENEMY || settings.unitType == UNITTYPE.BOSS) DrawPropertyField("canBeGrabbed");
                EditorGUI.indentLevel--;
            }
        ...
        public void showInfo(int id, UNITTYPE unitType)
        {
            ...
            switch (id)
            {
                ...
                case 7:
                    ...
                    if (unitType == UNITTYPE.ENEMY || unitType == UNITTYPE.BOSS) content += highlightItem("Defend Chance: ") + "The probability (0 - 100) that the enemy will successfully defend against an incoming attack." + newline;
                    if (unitType == UNITTYPE.ENEMY || unitType == UNITTYPE.BOSS) content += highlightItem("Defend Duration: ") + "The amount of time an enemy remains in the defence state after initiating defense." + newline;
                ...
                case 10:
                    ...
                    content = "\n";
                    if (unitType == UNITTYPE.ENEMY || unitType == UNITTYPE.BOSS) content += highlightItem("Load Random Name From List: ") + "Option to load a random enemy name from a txt file." + newline;
                    ...
                    if (unitType == UNITTYPE.ENEMY || unitType == UNITTYPE.BOSS) content += highlightItem("Unit Name: ") + "The unit's name as shown in the top near the enemy health bar." + newline;
